  ...
  if(cache_victim_d){
	...
	if(cache_dl1->last_blk_addr != 0){
		...
		if(cache_probe(cache_victim_d,baddr) == TRUE){ //if hit in cache_victim_d
			...
			cache_dl1->misses--; //compensating for additional l1_miss that will be counted when writing block to l1
  ...
  if(cache_victim_i){
	if(cache_il1->last_blk_addr != 0){
		...
		if(cache_probe(cache_victim_i,baddr) == TRUE){ //if hit in cache_victim_i
			...
			cache_il1->misses--; //compensating for additional l1_miss that will be counted when writing block to l1
