	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  cp->last_tagset = 0;
  cp->last_blk = NULL;
  cp->last_blk_addr = 0;  //ADDED FOR VICTIM
...
cache_access(struct cache_t *cp,	/* cache to access */
	     ...
	     bool results_on,		/* ADDED FOR VC flag to see if recording hit/miss */
	     bool writethrough_on)	/* ADDED FOR VC flag for write through htis/misses */
{
  byte_t *p = vp;
  ...
  if (repl->status & CACHE_BLK_VALID)
	...
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
      ...
      //cp->last_blk_addr = CACHE_MK_BADDR(cp, repl->tag, set); //ADDED FOR VICTIM
...
cache_flush(struct cache_t *cp,		/* cache instance to flush */
	    ...
	    bool results_on)		/* ADDED FOR VC flag for results */	
{
	  ...
	  if (blk->status & CACHE_BLK_VALID)
	      ...
	      if (blk->status & CACHE_BLK_DIRTY)
		{
		  ...
		  lat += cp->blk_access_fn(Write,
					   ...
					   cp->bsize, blk, now+lat, results_on);
		}
	    }
	}
    }
