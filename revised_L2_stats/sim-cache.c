...
dl1_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      ...
	      bool writethrough_on)	/* ADDED FOR VC flag for results for write through */	
{

  if(writethrough_on == false){
  	return cache_access(cache_dl2, cmd, baddr, NULL, bsize,
			  /* now */now, /* pudata */NULL, /* repl addr */NULL, false, false);//writethrough_on flag does not matter
  }
  ...
  if(cache_victim_d){
	...
	if(cache_dl1->last_blk_addr != 0){
		...
		if(cache_probe(cache_victim_d,baddr) == TRUE){ //if hit in cache_victim_d
			...
			cache_flush_addr(cache_victim_d, baddr, now, false); 
			// Write evicted dl1 block to vdl1 block address just vacated
			cache_access(cache_victim_d, Write, cache_dl1->last_blk_addr, NULL, bsize, now, NULL, NULL, false, false);
			//cache_victim_d->misses--; //don't want to count block swap as a miss
			//cache_dl1->hits--; //compensating for additional l1_miss that will be counted when writing block to l1
			// Write requested block from vdl1 to dl1
			cache_access(cache_dl1, Write, baddr, NULL, bsize, now, NULL, NULL, false, false); 
  ...
  if (cache_dl2)
    {
      /* access next level of data cache hierarchy */
      	return cache_access(cache_dl2, cmd, baddr, NULL, bsize,
			  /* now */now, /* pudata */NULL, /* repl addr */NULL, true, true);//writethrough_on flag does not matter
    }
  else
    {
...
//ADDED FOR VICTIM
//Misses in victim cache handled in il1_access_fn
static unsigned int			/* latency of block access */
vc_i_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
              md_addr_t baddr,		/* block address to access */
              int bsize,		/* size of block to access */
              struct cache_blk_t *blk,	/* ptr to block in upper level */
              tick_t now,		/* time of access */
	      ...
	      bool writethrough_on)	/* ADDED FOR VC flag for results for write through */
{
  ...
  if(cache_victim_i){
	if(cache_il1->last_blk_addr != 0){
		...
		if(cache_probe(cache_victim_i,baddr) == TRUE){ //if hit in cache_victim_i
			// Increment vil1 hits
			cache_victim_i->hits++;
			// Remove requested block from vil1
			cache_flush_addr(cache_victim_i, baddr, now, false); 
			// Write evicted il1 block to vil1 block address just vacated
			cache_access(cache_victim_i, Write, cache_il1->last_blk_addr, NULL, bsize, now, NULL, NULL, false, false);
			//cache_victim_i->misses--; //don't want to count block swap as a miss
			//cache_il1->hits--; //compensating for additional l1_miss that will be counted when writing block to l1
			// Write requested block from vil1 to il1
			cache_access(cache_il1, Write, baddr, NULL, bsize, now, NULL, NULL, false, false); 
			return 0;
		}
		else{
...
"      -cache:dl1 ul1:256:32:1:l -cache:dl2 ul2:1024:64:2:l\n"
  ...
  opt_reg_string(odb, "-cache:vc_i",
		 ...
		 &cache_victim_i_opt, "vc_i:32:l", /* print */ TRUE, NULL);
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
      ...
      else //data victim cache defined
	{
	      ...
	      if(sscanf(cache_victim_d_opt, "%[^:]:%d:%c", //removed bsize so now taking from d1
			name, &assoc, &c) != 3)
		fatal("bad l1 data victim cache parms: <name>:<assoc>:<repl>");
  ...
  if (!mystricmp(cache_il1_opt, "none"))
      ...
      else //instruction victim cache defined
	{
	      ...
	      if(sscanf(cache_victim_i_opt, "%[^:]:%d:%c", //removed bsize and taking from l1
			name, &assoc, &c) != 3)
		fatal("bad l1 instruction victim cache parms: <name>:<assoc>:<repl>");
...
#define __READ_CACHE(addr, SRC_T)					\
  ((dtlb								\
    ? cache_access(dtlb, Read, (addr), NULL,				\
		   sizeof(SRC_T), 0, NULL, NULL, true, true)			\
    : 0),								\
   (cache_dl1								\
...
#define __WRITE_CACHE(addr, DST_T)					\
  ((dtlb								\
    ? cache_access(dtlb, Write, (addr), NULL,				\
		   sizeof(DST_T), 0, NULL, NULL, true, true)			\
    : 0),								\
   (cache_dl1								\
...
{
  if (dtlb)
    cache_access(dtlb, cmd, addr, NULL, nbytes, 0, NULL, NULL, true, true);
  if (cache_dl1)
    cache_access(cache_dl1, cmd, addr, NULL, nbytes, 0, NULL, NULL, true, true);
...
#define SYSCALL(INST)							\
  (flush_on_syscalls							\
   ? ((dtlb ? cache_flush(dtlb, 0, true) : 0),				\
      (cache_dl1 ? cache_flush(cache_dl1, 0, true) : 0),			\
      (cache_dl2 ? cache_flush(cache_dl2, 0, true) : 0),			\
...
sim_main(void)
{
  ...
  while (TRUE)
	...
	cache_access(itlb, Read, IACOMPRESS(regs.regs_PC),
		     NULL, ISCOMPRESS(sizeof(md_inst_t)), 0, NULL, NULL, true, true);
      if (cache_il1)
