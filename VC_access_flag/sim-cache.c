  ...
  if(cache_victim_d){
	...
	if(cache_dl1->last_blk_addr != 0){
		...
		if(cache_probe(cache_victim_d,baddr) == TRUE){ //if hit in cache_victim_d
			...
			cache_access(cache_victim_d, Write, cache_dl1->last_blk_addr, NULL, bsize, now, NULL, NULL, false);
			...
			cache_access(cache_dl1, Write, baddr, NULL, bsize, now, NULL, NULL, false); 
  ...
  if(cache_victim_i){
	if(cache_il1->last_blk_addr != 0){
		...
		if(cache_probe(cache_victim_i,baddr) == TRUE){ //if hit in cache_victim_i
			...
			cache_access(cache_victim_i, Write, cache_il1->last_blk_addr, NULL, bsize, now, NULL, NULL, false);
			...
			cache_access(cache_il1, Write, baddr, NULL, bsize, now, NULL, NULL, false); 
			return 0;
		}
		else{
...
{
  if (dtlb)
    cache_access(dtlb, cmd, addr, NULL, nbytes, 0, NULL, NULL, true);
  if (cache_dl1)
    cache_access(cache_dl1, cmd, addr, NULL, nbytes, 0, NULL, NULL, true);
...
sim_main(void)
{
  ...
  while (TRUE)
	...
	cache_access(itlb, Read, IACOMPRESS(regs.regs_PC),
		     NULL, ISCOMPRESS(sizeof(md_inst_t)), 0, NULL, NULL, true);
      if (cache_il1)
